...
export function useInvalidateMatches() {
  ...
  return {
    ...
    invalidateList: () => queryClient.invalidateQueries({ queryKey: matchKeys.lists() }),
    invalidateDetail: (id: number) => queryClient.invalidateQueries({ queryKey: matchKeys.detail(id) }),
  }
}
